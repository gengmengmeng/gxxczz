...
namespace MisFrameWork3.Areas.FWBase.Controllers
{
    ...
    public class UsersController : FWBaseController
    {
        ...
        public ActionResult ActionAdd()
        {
            ...
            */
            ...
            try
            {
                ...
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户增加session已开启");
                ...
                session.Commit();
                session.Close();
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户增加session已关闭");
            }
            catch (Exception e)
            {
                session.Rollback();
        ...
        public ActionResult ActionEdit()
        {
            ...
            {
                session.BeginTransaction();
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户修改session已开启");
                ...
                session.Commit();
                session.Close();
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户修改session已关闭");
            }
            catch (Exception e)
            {
                session.Rollback();
        ...
        public ActionResult ActionChangeState()
        {
            ...
            if (state == 0)//启用的话，要检查一下单位信息，看单位有没被停用
            {
                try
                {
                    UnCaseSenseHashTable recordUser = DbUtilityManager.Instance.DefaultDbUtility.GetOneRecord("FW_S_USERS", id, "USER_ID,COMPANY_ID,(SELECT c.DISABLED as C_DISABLED FROM FW_S_COMAPANIES  c where c.ID=COMPANY_ID) as C_DISABLED");
                    if (recordUser == null)
            ...
            {
                session.BeginTransaction();
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户状态修改session已开启");
                DbUtilityManager.Instance.DefaultDbUtility.Execute("update  FW_S_USERS set DISABLED=" + state + " where USER_ID=\'" + id + "\'");
                session.Commit();
                session.Close();
                Log4NetHelp.WriteDebugLog(DateTime.Now.ToString() + "用户状态修改session已关闭");
            }
            catch (Exception e)
            {
                session.Rollback();
        ...
        public ActionResult JsonRoles(string user_id)
        {
            ...
            //cdtMain.AddSubCondition("AND", "DISABLED", "=", "0");
